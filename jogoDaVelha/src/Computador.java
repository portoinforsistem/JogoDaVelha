...
//    @Override
    public char[][] jogar(char[][] mat, int qtJogada) { //override da função de jogada do jogador
        Random random = new Random(); //valor randomico a cada jogada, para quando não há condição específica
        ...
        while (true) {
            if (testeJogadaOfensiva(mat) == 1) { //sempre testado primeiro, pois caso o compuador possa vencer, ele vai
                mat = jogadaOfensiva(mat); //executa a jogada ofensiva
                return mat;
            } else if (testeJogadaDefensiva(mat) == 1) { //impede o jogador de completar uma sequência de três no tabuleiro
                mat = jogadaDefensiva(mat); //executa a jogada defensiva
                return mat;
            } else if (qtJogada == 1) { //momento decisivo, se o jogador joga na borda, o computador TEM que jogar no meio
                if (priJogBorda(mat) == 1) { //teste se a jogada foi na borda
//            prossegue jogando no meio
                    mat[1][1] = 'X';
                    return mat;
                } else if (priJogCentro(mat) == 1) { //teste se a jogada foi no centro
                    //joga na borda
                    mat[0][0] = 'X';
                    return mat;
                }
...
//            linha = (int) (Math.random() * 2);
//            coluna = (int) (Math.random() * 2);
                if (mat[linha][coluna] == '#') {
                    mat[linha][coluna] = 'X';
                    return mat;
                }
    ...
    //método que informa se a jogada foi feita na borda
    private int priJogBorda(char[][] mat) {
        for (int i = 0; i < 3; i++) {
            for (int j = 0; j < 3; j++) {
                if (i == 1 && j == 1) {
                    continue;
    ...
    public int priJogCentro(char[][] mat) {
        if ((mat[0][0] == '#' && mat[0][1] == '#' && mat[0][2] == '#') && mat[1][1] == 'O') {
            return 1;
        } else {
            return 0;
        }
    ...
    /*aqui o jogador esta prestes a usar a tatica de marcação de posições em V ou L, tendo 2/3 de sua estrategia já formada, ou seja, 
    após a sua segunda jogada. Isso gera a chance de duas jogadas vitoriosas possiveis posteriormente. 
    Jogando previamente numa dessas posições, ao invés de prosseguir ofensivamente ou aleatoriamente, elimina as chances dele.*/
